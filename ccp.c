...
int main(int argc, char **argv){
	...
	int t0, tf, w, n, wsum, nlen, nerr, nlay, ncols, max=MAX, i, j, k, fz;
	...
	/* Arrays de l'FFT */
	fftw_complex fftin[MAX], fftout[MAX], fftout2[MAX], phase;
	...
	/* ------------------Inicialització de les FFT------------------- */
	planf = fftw_plan_dft_1d(MAX, fftin, fftout, FFTW_FORWARD, FFTW_ESTIMATE);
	planb = fftw_plan_dft_1d(MAX, fftout, fftout2, FFTW_BACKWARD, FFTW_ESTIMATE);
	...
	while(fscanf(list_file, "%s", rf) == 1){
		...
		fftw_execute(planb);
		/* Renormalització de les dades, que no s'escalen amb la fft */
		/* Càlcul de la fase instantània (senyal analítica / amplitud
		 * instantània) */
		for(j=0;j<MAX;j++){
			fftout2[j] /= MAX;
			fftout2[j] /= cabs(fftout2[j]);
		}
		...
		for(j=0; j<nlay; j++){
			...
			//a = ((1/us)*(1/us)*dzf)/sqrt((1/us)*(1/us)-p*p);
			//b = ((1/up)*(1/up)*dzf)/sqrt((1/up)*(1/up)-p*p);
			//dt = a-b;
			a = sqrt(1/(us*us)-p*p);
			b = sqrt(1/(up*up)-p*p);
			dt = (a-b)*dzf;
			/* Càlcul de l'amplitud dins l'interval de temps */
			/*a = (acctime+dt)/delta-acctime/delta;*/
			/*k = round((acctime+a/2)/delta);*/	
			k = round(acctime/delta)+1;
			acctime += dt;
			phase = fftout2[k];
			amp = array[k];
			...
			/* Factor de suavitzat amplitud-profunditat */
			q = pow(zf, 0.25);
	...
	for(i=0; i<nlay; i++){
		for(w=0; w<ncols; w++){
			...
			if(fabs(perfil[w][i])>a){a=fabs(perfil[w][i]);}
			absphase[w][i] = fabs(pphase[w][i])/pnorm[w][i];
			absphase[w][i] = pow(absphase[w][i],nu);
