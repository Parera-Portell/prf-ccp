 ...
 * llarg del raig sísmic; a-b-c: variables auxiliars per càlculs extensos;
...
void proj(float dist, float az0, float az1, float *x, float *y){
	...
	*x = dist*cos(azdiff);
	*y = dist*sin(azdiff);
	/* x és negativa si la projecció queda abans del punt inicial del 
	 * perfil */
}
...
int main(int argc, char **argv){
	...
	int t0, tf, w, n, wsum, nlen, nerr, nlay, ncols, max=MAX, i, j, k, l;
	float delta, dx, dz, dep, depmin, depmax, beg, p, array[MAX], z, mvp, 
	mvs, inilat, inilon, finlat, finlon, len, azim, stla, stlo, baz, hw, 
	az0, az1, dist0, dist1, x, y, xi, yi, lati, loni, ds, dt, a, b, c,
	up, us, zf, psph, acctime, amp, latj, lonj, fzr, wl;
	...
	/* ---Llegir i transformar model de gradient a model de capes---- */
	...
	fclose(mod_file);
	/* Aquí comença la transformació a un model de capes */
	n=0;
	for(w=0; w<=nlay; w++){
		z=dz*w;
		i=0; mvp=0; mvs=0;
	...
	float perfil[ncols][nlay], pnorm[ncols][nlay];
	for(i=0; i<ncols; i++){
		for(w=0; w<nlay; w++){
			perfil[i][w] = 0;
			pnorm[i][w] = 1;
	...
	list_file = fopen(rflist, "r");
	w = 0;
	printf("\nProcessing RF number...");
	while(fscanf(list_file, "%s", rf) == 1){
		...
		/* Comprovar si l'estació o el raig sísmic pot passar pel 
		 * perfil o la seva àrea de mostreig lateral. Es calcula en
		 * funció de la variació dels punts projectats x/y en fer un 
		 * recorregut d'1 km seguint el backazimuth del raig sísmic */
		i = 0;
		garc(inilat,inilon,stla,stlo,&dist0,&az0);
		proj(dist0,az0,azim,&x,&y);
		proj(1,baz,azim,&xi,&yi);
		xi += x;
		yi += y;
		if (fabs(y)<hw & x>=0 & x<=len){i = 1;}
		
		else if(fabs(y)>hw & x>=0 & x<=len){
			if(y>0 & yi<y){i = 1;}
			else if(y<0 & yi>y){i = 1;}
		}
		 
		else if(fabs(y)<hw & x<0){
			if(xi>x){i = 1;}
		}
		 
		else if(fabs(y)<hw & x>len){
			if(xi<x){i = 1;}
		}
		 
		else if(fabs(y)>hw & x<0){
			if(y>0 & yi<y & xi>x){i = 1;}
			else if(y<0 & yi>y & xi>x){i = 1;}
		}
		...
		if(i==1){
			for(j=0; j<nlay; j++){
				...
				a = -EARTHRAD*log((EARTHRAD-j*dz)/EARTHRAD);
				b = -EARTHRAD*log((EARTHRAD-(j+1)*dz)/EARTHRAD);
				zf = b-a;
				/* Càlcul de la distància epicentral entre els piercing
				 * points del raig sísmic a z i z+1. S'utilitza el
				 * paràmetre de raig en coordenades esfèriques. */
				psph = p/(KM2DEG*DEG2RAD);
				c = (emod.vs[j]*((j+1)*dz+j*dz)/2);
				a = psph/sqrt(c*c-psph*psph);
				b = ((j+1)*dz-j*dz)/(((j+1)*dz+j*dz)/2);
				ds = a*b;
				/* Càlcul del temps de viatge entre z i z+1 */
				a = sqrt(1/us*us-p*p);
				b = sqrt(1/up*up-p*p);
				dt = (a-b)*zf;
				/* Càlcul de l'amplitud mitjana de l'RF dins l'interval
				 * de temps */
				amp = 0;
				a = round(acctime/delta);
				b = round(acctime+dt/delta);
				for(k=a; k<=b; k++){
					amp += array[k];
				}
				amp /= (b-a);
				 ...
				 * El periode (P) de la FZ == 1. wl = P*v */
				ds *= RAD2DEG*DEG2KM;
				proj(ds,baz,azim,&xi,&yi);
				x += xi;
				y += yi;
				zf = -EARTHRAD*log((EARTHRAD-(j+1)*dz)/EARTHRAD);
				wl = 1*us;
				fzr = sqrt(0.5*wl*zf+0.0625*wl*wl);
				if(x+fzr>=0 & x-fzr<=len & fabs(y)<=hw){
					n = round(fzr/dx);
					l = round(x/dx);
					for(k=l-n; k<=l+n; k++){
						 ...
						 * -->*exp(-x**2/2*std**2) 
						 * a --> fzr es multiplica pel número d'FZ (1) */
						if(k>=0 & k<=ncols){
							/* Factor 1: Suavitzat gaussià */
							a = 0.5*1*fzr; /* std */
							b = -0.5*((l-k)*dx)*((l-k)*dx)/(a*a);
							c = exp(b)/a*sqrt(2*3.14159);
							/* Factor 2: Suavitzat amplitud-profunditat */
							a = pow((j+1)*dz, 0.25);
						}
					}
