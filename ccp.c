...
void main(int argc, char **argv){
	...
	for(w=0; w<nlay; w++){
		...
		i=0; mvp=0; mvs=0;
		if(w==0){
			emod.vp[w]=emod0.vp0[0];
			emod.vs[w]=emod0.vs0[0];
		}
		else{
			...
			if(emod0.dep0[n]==z){
				...
				y0=emod0.vp0[n-1];
				y1=emod0.vp0[n];
				emod.vp[w] = y0 + ((y1-y0)/(x1-x0)) * (z - x0);
				y0=emod0.vs0[n-1];
				y1=emod0.vs0[n];
				emod.vs[w] = y0 + ((y1-y0)/(x1-x0)) * (z - x0);
			}
		}
	}
	...
	for(i=0; i<nlay; i++){
		for(w=0; w<ncols; w++){
			perfil[i*ncols+w] /= sqrt(pnorm[i*ncols+w]);
			absphase[i*ncols+w] = pow(fabs(pphase[i*ncols+w])/pnorm[i*ncols+w],nu);
			/* Suavitzat per files, només una direcció */
			if(w>0){
